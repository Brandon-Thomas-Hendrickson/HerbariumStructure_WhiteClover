...
sed -E 's,.*/,,' /work/calicraw/Projects/HerbariumStructure/file_lists/herbarium_GLUE_spain/bam_list_GLUESPAIN.txt > /work/calicraw/Projects/HerbariumStructure/file_lists/herbarium_GLUE_spain/bam_list_GLUESPAIN_TEMP.txt

source /home/calicraw/miniconda3/etc/profile.d/conda.sh
conda activate alignment_conda

#Extract bam and TE annotations
for i in $(cat /work/calicraw/Projects/HerbariumStructure/file_lists/herbarium_GLUE_spain/bam_list_GLUESPAIN_TEMP.txt); do
    bedtools intersect -bed -abam /work/calicraw/Projects/HerbariumMicrobes/Aligned/endogenous/GLUE/processed_bam/$i -b /work/calicraw/Genome/UTM_Trep_v1.0_haploid_reference_repeatMasker_CP.bed -wa -wb > /work/calicraw/Projects/HerbariumStructure/transposons/output/${i%_marked.bam}'.txt';
