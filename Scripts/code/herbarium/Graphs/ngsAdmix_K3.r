...
# Load the data
herb_data <- read.csv("Github_Projects/HerbariumStructure_WhiteClover/Data/herbarium/Input/herbarium_structure_dataframe_542025.csv",header=TRUE)
...
long_data <- filtered_data %>%
  select(Samp, Country, K3Q1S1964, K3Q2S1964, K3Q3S1964) %>%
  pivot_longer(cols = starts_with("K3"), names_to = "Cluster", values_to = "Proportion") %>%
...
ngsadmix_plot_T1 <- ggplot(long_data_T1, aes(x = Samp, y = Proportion, fill = Cluster)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size=12,color="black"),
    axis.title.y = element_text(size=12,color="black"),  # Hide x-axis text for clarity
...
ngsadmix_plot_All <- ggplot(long_data_All, aes(x = Samp, y = Proportion, fill = Cluster)) +
  ...
  theme_minimal() + xlab("North → South") +
  theme(
    legend.position = "none",
