...
gamma_model <- glm(Heterozygosity ~ Year, data = herb_N[herb_N$Year <= 1997, ], family = gaussian)
summary(gamma_model)
simulation_output <- simulateResiduals(fittedModel = gamma_model, plot = TRUE)
# Optional: Perform additional tests
testDispersion(simulation_output)
...
gamma_model <- glm(Heterozygosity ~ Year,data = herb_S[herb_S$Year <= 1997, ], family = gaussian)
summary(gamma_model)
# Create a new data frame for predictions
...
gamma_model <- glm(Heterozygosity ~ Year, data = herb_M[herb_M$Year <= 1997, ], family = gaussian)
summary(gamma_model)
simulation_output <- simulateResiduals(fittedModel = gamma_model, plot = TRUE)
# Optional: Perform additional tests
testDispersion(simulation_output)
...
plot_NS <- ggplot(data = herb[herb$Year <= 1997, ], aes(x = Year, y = Heterozygosity)) +
    geom_point(data = herb,color="black",alpha=0.4) +
geom_line(data = prediction_data, aes(x = Year, y = Predicted_Heterozygosity), color = "black", size = 2, linetype = "dashed") +
geom_line(data = prediction_data_N, aes(x = Year, y = Predicted_Heterozygosity), color = "blue", size = 2, linetype = "solid") +
geom_line(data = prediction_data_S, aes(x = Year, y = Predicted_Heterozygosity), color = "red", size = 2, linetype = "solid") + 
geom_line(data= prediction_data_M, aes(x=Year, y=Predicted_Heterozygosity), color="lightblue", size=2, linetype="solid") +
  xlab("Year") +
theme_light() + theme(plot.title = element_text(hjust = 0.5), axis.text = element_text(size = 12, color = "black"), axis.title = element_text(size = 12, color = "black"))
...
geom_line(data = prediction_data_N, aes(x = MaxK3_LD, y = Predicted_Heterozygosity), color = "blue", size = 2, linetype = "solid") +
geom_line(data = prediction_data_S, aes(x = MaxK3_LD, y = Predicted_Heterozygosity), color = "red", size = 2, linetype = "solid") + 
geom_line(data= prediction_data_M, aes(x=MaxK3_LD, y=Predicted_Heterozygosity), color="lightblue", size=2, linetype="solid") +
  xlab("Max Q") +
theme_light() + theme(plot.title = element_text(hjust = 0.5), axis.text = element_text(size = 12, color = "black"), axis.title = element_text(size = 12, color = "black"))
...
ggsave("Figure3.pdf", plot = combined_panelA, width = 8, height = 4, units = "in")
