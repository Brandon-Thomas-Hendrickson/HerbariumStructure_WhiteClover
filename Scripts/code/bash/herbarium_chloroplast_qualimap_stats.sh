...
OUTPUT_CSV="/work/calicraw/Projects/HerbariumStructure/aligned/qualimap_reports/herbarium_chloroplast/HERBARIUM_CHLOROPLAST_QUALI_STATS_withdownsample.csv"
BAM_DIR="/work/calicraw/Projects/HerbariumStructure/aligned/processed/herbarium/chloroplast"
QUALIDIR="/work/calicraw/Projects/HerbariumStructure/aligned/qualimap_reports/herbarium_chloroplast/qualistats_reports"
...
sed -i 's,/work/calicraw/Projects/HerbariumStructure/aligned/processed/herbarium/chloroplast/,,g' herbarium_bam_list.txt
...
for BAM_FILE in $(cat herbarium_bam_list.txt); do
    ...
    MEAN_COVERAGE=$(cat "$STATS_OUTPUT" | grep "CP125853.1" | awk '{print $4}' | sed 's.,..g')
    STD_COVERAGE=$(cat "$STATS_OUTPUT" | grep "CP125853.1" | awk '{print $5}' | sed 's.,..g')
